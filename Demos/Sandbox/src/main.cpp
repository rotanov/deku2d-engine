...
public:
	...
	void ProcessEvent(const Deku2D::CEvent &AEvent)
	{
		...
		else if (AEvent.GetName() == "KeyDown" && AEvent.GetData<Uint16>("Sym") == SDLK_d)
		{
			...
			{
				CObject** o = (CObject**) TypeInfo::GetTypeInfo("CGameObject")->GetProperty("Children")->GetValue(Deku2D::CEngine::Instance()->RootGameObject, i);
				Log("ASDALKDJAKD", "%s", (*o)->GetName().c_str());
			}
