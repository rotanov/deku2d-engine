...
struct Vector2
{
	...
	// ¬ некотором смысле аналог векторного произведени€ дл€ 2х мерного случа€
	__INLINE scalar operator ^ (const Vector2 &V) const
	{
		return x * V.y - y * V.x;
	}
...
/**
*	–ассто€ние от точки до пр€мой вычисл€ет эта функци€.
*	v1, v2 - координаты любых двух (возможно не совпадающих)
*		точек, принадлежащих пр€мой.
*	p - та сама€ точка, рассто€ние от которой мы ищем.
*	TODO : оптимизировать еЄ нужно.
*/
...
/**
*	MatrixNM - произвольна€ матрица. M строк, N столбцов
*/ 
...
/**
*	SqareEq(...) Ёта детка решает нам квдратное уравнение.
*	» запихивает корни в t0 и t1
*	a b c - соответствующие коэффициенты квадратного уравнени€
*/

bool SqareEq( scalar a, scalar b, scalar c, scalar &t0, scalar &t1);

/**
*	CalcFrustumVertices(...) - функци€ вычисл€ет координаты вершин
*	усеченной пирамиды, образуемой пирамидой камеры и 2м€ плоскост€ми отсечени€.
*/
...
/**
*	PointsPlaneSide(...) - фунци€ провер€ет лежат ли все точки
...
// непон€тна€ функци€. походу € еЄ откуда-то рипанул, надо разобратьс€
