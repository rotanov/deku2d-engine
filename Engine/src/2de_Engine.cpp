...
bool CEngine::Init()
{
	...
	#define WIN32_LEAN_AND_MEAN		
	{
		...
		delete [] pathexe;
		ShowWindow(hwnd, STATE_HIDE_CONSOLE_WINDOW);  // ¬ ранней версии SDL всегда вылазило окно консоли, потом этот кос€к убрали, а мой фикс тут осталс€
	...
	while(SDL_PollEvent(&event)) // TODO: Look here!!!!!! http://osdl.sourceforge.net/main/documentation/rendering/SDL-inputs.html
	{
		...
		switch(event.type)
		{
			case SDL_KEYDOWN:
			{
				...
				// √лобальна€ рекци€ на escape! —лишком большой хардкод, но пока сойдЄт. ѕотом - либо вывести в опцию, либо убрать и предоставить программисту право выбора
			...
			{
				// «десь можно раздавать позицию мыши всем попросившим.
...
// 				> ћожно вынести опрос координат курсора в отдельный поток. 
// 
// 				но применительно дл€ SDL, можно установить глобальный фильтр обработчика где будет, вызыватьс€ тво€ функци€ отрисовки курсора (€ делал именно так, и не тормозит)
// 
...
// 			}
// 
// 			это не дословный код но обща€ иде€ именно така€.
//------------------
// + иде€ от мен€ - можно пытатьс€ предсказывать движение курсора, т.е. где он окажетс€, пока мы рисуем кадры под 60FPS
